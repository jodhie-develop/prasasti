...
export async function askOpenAI({ message, history = [], extraContext = "" }) {
    ...
    const enableWebSearch = process.env.ENABLE_WEB_SEARCH !== "false"; // on by default
    ...
    const response = await fetch("https://api.openai.com/v1/responses", {
        ...
        body: JSON.stringify({
            ...
            // Lets the model search the web for current events (e.g. geopolitical situations,
            // fuel surcharges, port congestion news) instead of refusing or guessing.
            // Set ENABLE_WEB_SEARCH=false in Vercel env vars to turn this off (saves cost).
    ...
    if (!reply) {
        const debugSnippet = JSON.stringify(data).slice(0, 800);
        console.error("Unexpected OpenAI response shape:", debugSnippet);
        throw new Error(
            `OpenAI tidak mengembalikan jawaban yang valid. Status: ${data.status || "unknown"}. Raw: ${debugSnippet}`
        );
    }
...
/**
 * Manually walks the Responses API output array to collect the assistant's text.
 * NOTE: `output_text` is a convenience property added only by OpenAI's official
 * SDKs — it does NOT exist on the raw REST/fetch JSON response, so we build it
 * ourselves here instead of reading `data.output_text`.
 */
function extractOutputText(data) {
        ...
        if (item.type === "message" && Array.isArray(item.content)) {
            for (const block of item.content) {
                if (block.type === "output_text" && typeof block.text === "string") {
                    ...
                    // Collect any web citations attached to this text so we can list sources.
    ...
    if (sources.length > 0) {
        const uniqueSources = Array.from(new Map(sources.map((s) => [s.url, s])).values()).slice(0, 4);
        const sourceList = uniqueSources.map((s) => `- ${s.title}: ${s.url}`).join("\n");
        reply += `\n\nSumber:\n${sourceList}`;
    }
